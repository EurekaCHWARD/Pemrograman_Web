...
namespace Praktikum\Routes;

include "/xampp/htdocs/Praktikum/Controller/ProductController.php";

use Praktikum\Controller\ProductController;
...
class ProductRoutes
{
    public function handle($method, $path)
    {
        ...
        if ($method === 'GET' && $path === "/modul5/products") {
            $controller = new ProductController();
            echo $controller->index();
        }

        // JIKA REQUEST METHOD GET DAN PATH MENGANDUNG '/api/product/'
        if ($method === 'GET' && strpos($path, "/modul5/products/") === 0) {
        ...
        if ($method === 'POST' && $path === "/modul5/products") {
            $controller = new ProductController();
            echo $controller->insert();
        }

        // JIKA REQUEST METHOD PUT DAN PATH MENGANDUNG '/api/product/'
        if ($method === 'PUT' && strpos($path, "/modul5/products/") === 0) {
            // Extract ID dari path
            $pathParts = explode('/', $path);
            $id = $pathParts[count($pathParts) - 1];
            $controller = new ProductController();
        ...
        if ($method === 'DELETE' && strpos($path, "/modul5/products/") === 0) {
            // Extract ID dari path
            $pathParts = explode('/', $path);
            $id = $pathParts[count($pathParts) - 1];
            $controller = new ProductController();
